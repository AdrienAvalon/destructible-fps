...
| TLS lifecycle | Replacement key and certificate mismatch, then coherent rotation during an active session | `tls_reload_keeps_the_previous_deadline_until_a_valid_pair_is_installed`, `tls_rotation_changes_future_handshakes_without_disrupting_an_active_session`, and the standalone rotation process test | Pass |
